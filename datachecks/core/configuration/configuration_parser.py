...
def parse_data_source_yaml_configurations(
    ...
    data_source_names = [data_source["name"] for data_source in data_source_yaml_configurations]
    if len(data_source_names) != len(set(data_source_names)):
        raise DataChecksConfigurationError(f"Duplicate data source names found in {data_source_names} configuration")
